...
get_detail_func <- function(GSE_ID_each){
  ...
  phenoDat <- pData(eSet)
  file_path <- paste(save_path, "all_lc_", GSE_ID_each_cated, ".csv", sep="")
  if (!file.exists(file_path)) {
    file.create(file_path)
  }
  write.table(phenoDat, file=file_path, sep=",", row.names=T, quote=FALSE)  # step2 Annotate the probe in the platform file from which the data was obtained
  gpl=eSet@annotation
  checkGPL(gpl)
  printGPLInfo(gpl)
